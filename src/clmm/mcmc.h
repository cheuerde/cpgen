...
#pragma omp parallel
	...
	for(int i=0;i<list_of_design_matrices.size();i++){
		...
		model_effects.push_back(effects(list_of_design_matrices[i], ycorr.data(), Rcpp::List(par_random[i]),
					       	&var_e, var_y, n_random, niter, burnin, list_of_ginverse[i])); 
