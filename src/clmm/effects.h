...
class effects {
	...
	public:
		...
		effects(SEXP design_matrix_from_MCMC, double * ycorr_from_MCMC, Rcpp::List list_from_MCMC, 
			double * var_e_from_MCMC, double var_y_from_MCMC,int total_number_effects_from_MCMC,
			int niter_from_MCMC, int burnin_from_MCMC, SEXP ginverse_from_MCMC);
		inline void initialize(base_methods_abstract*& base_fun);  
		inline void sample_effects(sampler& mcmc_sampler,base_methods_abstract*& base_fun, 
					   mp_container& thread_vec, int& niter_from_MCMC); 
...
void effects::initialize(base_methods_abstract*& base_fun) {
	...
	if(sparse_or_dense == "sparse") {
		...
		if((method == "ridge_ginverse") && (sparse_or_dense_ginverse == "sparse")) {

			my_functions = new function_ridge_ginv<MapSparseMatrixXd, MapSparseMatrixXd>(design_matrix, ginverse); 

		}

		if((method == "ridge_ginverse") && (sparse_or_dense_ginverse == "dense")) {

			my_functions = new function_ridge_ginv<MapSparseMatrixXd, MapMatrixXd>(design_matrix, ginverse); 
		...
		if((method == "ridge_ginverse") && (sparse_or_dense_ginverse == "sparse")) {
			...
			my_functions = new function_ridge_ginv<MapMatrixXd, MapSparseMatrixXd>(design_matrix, ginverse); 

		}

		if((method == "ridge_ginverse") && (sparse_or_dense_ginverse == "dense")) { 
			
			my_functions = new function_ridge_ginv<MapMatrixXd, MapMatrixXd>(design_matrix, ginverse); 
...
void effects::sample_effects(sampler& mcmc_sampler,base_methods_abstract*& base_fun, 
			     mp_container& thread_vec, int& niter_from_MCMC) {
