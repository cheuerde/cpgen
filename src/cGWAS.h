...
template<class T> 
void GWA<T>::initialize(SEXP y_R_in, SEXP M_R_in, SEXP V_R_in, SEXP V2_R_in, SEXP X_R_in, bool has_dom, bool second_transform_in, bool verbose_R_in) {
			...
			if ( ! Progress::check_abort() ) {
				...
				if(second_transform) {
					...
					if(dom) {
						...
						X_marker.col(1) = V * (V2 * (1.0 - abs(M.col(i).array())).matrix());
					...
					if(dom) {
						...
						X_marker.col(1) = V * (1.0 - abs(M.col(i).array())).matrix();
